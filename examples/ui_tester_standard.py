...
class UITestReport:
    ...
    def __init__(self, test_name: str):
        ...
        self.start_time = 0
        self.end_time = 0
        self.total_time: float = 0.0  # 标准操作总耗时
    ...
    def complete_test(self):
        ...
        if self.total_time > 0:
            logger.info(f"\n性能数据:")
            logger.info(f"  标准操作总耗时: {self.total_time:.4f}秒")
...
class StandardUITester:
    ...
    async def find_element_by_text(self, context, text_content: str, tag_names: Optional[List[str]] = None, exact_match: bool = False, interactive_only: bool = True) -> Optional[int]:
        """通过文本内容查找元素
        ...
        for index, element in dom_state.selector_map.items():
            ...
            # 检查是否隐藏
            if await self.is_element_hidden(element):
                continue
    ...
    async def find_input_element(self, context, input_type: Optional[str] = None, placeholder: Optional[str] = None) -> Optional[int]:
        """查找输入框元素
    ...
    async def run_ui_test(self):
        """运行UI测试"""
        logger.info("启动标准UI测试（不使用缓存）...")
        
        # 创建测试报告
        self.report = UITestReport("登录系统标准测试")
        ...
        try:
            ...
            try:
                # 使用标准方法查找用户名输入框
                username_index, standard_time = await self.measure_performance(
                    context, 
                ...
                # 记录性能数据
                self.report.total_time += standard_time
            ...
            try:
                # 查找密码输入框
                password_index, standard_time = await self.measure_performance(
                    context,
                    self.find_input_element,
            ...
            try:
                # 使用标准方法查找登录按钮
                login_button_index, standard_time = await self.measure_performance(
                    context,
            ...
            try:
                ...
                target_element_index, standard_time = await self.measure_performance(
                    context,
                    self.find_element_by_text,
                ...
                if target_element_index is None:
                    ...
                    for idx, element in dom_state.selector_map.items():
                        if getattr(element, 'is_interactive', False) and not await self.is_element_hidden(element):
                            element_text = ""
                            if hasattr(element, 'get_all_text_till_next_clickable_element'):
                                element_text = element.get_all_text_till_next_clickable_element()
                            elif hasattr(element, 'text'):
            ...
            try:
                # 使用标准方法验证
                home_text_index, standard_time = await self.measure_performance(
                    context,
                    self.find_element_by_text,
...
async def batch_run_tests(num_runs=1):
    """批量运行测试多次以获取更稳定的性能数据"""
    logger.info(f"开始批量运行标准UI测试 ({num_runs}次)...")
    
    success_count = 0
    tester = StandardUITester()
